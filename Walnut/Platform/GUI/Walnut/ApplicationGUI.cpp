...
#include "Walnut/Embed/WindowImages.embed"
	...
	void Application::Init()
	{
		...
		if (!m_Specification.IconPath.empty())
		{
			...
			icon.pixels = stbi_load_from_memory((const stbi_uc*)g_WalnutIcon, sizeof(g_WalnutIcon), &icon.width, &icon.height, &channels, 4);
			glfwSetWindowIcon(m_WindowHandle, 1, &icon);
			stbi_image_free(icon.pixels);
		}
		...
		// Load images
		{
			if(!m_Specification.TopbarIconPath.empty()) // gives people the ability to pass images as icons
			{
				int w, h;
				void* data = stbi_load(m_Specification.TopbarIconPath.string().c_str(), &w, &h, &channels, 4);;
	...
	void Application::UI_DrawTitlebar(float& outTitlebarHeight)
	{
		...
		// Logo
		{
			const int logoWidth = 40;// m_LogoTex->GetWidth();
			const int logoHeight = 40;// m_LogoTex->GetHeight();
			const ImVec2 logoOffset(16.0f + windowPadding.x, 10.0f + windowPadding.y + titlebarVerticalOffset);
